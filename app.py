...
st.set_page_config(page_title="GhostBot", page_icon="💀", layout="centered")
...
#  ÁREA DE INTERAÇÃO
prompt = st.chat_input("Digite qualquer dúvida para o GhostBot...")
...
with st.sidebar:
    ...
    if st.button("🎤 Falar pelo microfone"):
        ...
        if texto_ouvido:
            st.success(f"O texto foi reconhecido com sucesso")
    
            #adicionar a mensagem do usuario no chat
            st.session_state.messages.append({'role':'user', 'content':texto_ouvido})

            #exibir a msg do chat
            ...
            st.session_state.messages.append({'role':"assistent", "content":resposta_ia})        

            st.rerun()

        else:
            st.warning("Não foi possivel reconhecer nenhuma fala, tente novamente")    
